...
class User {
	...
	public function login($username = null, $password = null, $remember = false){
		...
		if (!$username && !$password && $this->exists()) {
			...
			if ($user) {
				if ($this->data()->ldap == 0) {
					$passedValidation = (password_verify($password,$this->data()->password));
				} else {
					//ldap procedures
					$settingsQ = $this->_db->query("SELECT ldap_address, ldap_prefix FROM settings");
					$settings = $settingsQ->first();
					$ldapCon = ldap_connect($settings->ldap_address);
					$ldapuser  = $settings->ldap_prefix."\\".$username;
					$ldapBinded = ldap_bind($ldapCon, $ldapuser, $password);
					if ($ldapBinded) {
						$passedValidation = $ldapBinded;
					}
				}
				if ($passedValidation) {
					Session::put($this->_sessionName, $this->data()->id);
		...
		if (!$email && !$password && $this->exists()) {
			...
			if ($user) {
				if ($this->data()->ldap == 0) {
					...
					$settings = $settingsQ->first();
					$ldapCon = ldap_connect($settings->ldap_address);
					$ldapuser  = $settings->ldap_prefix."\\".$email;
					$ldapBinded = ldap_bind($ldapCon, $ldapuser, $password);
					if ($ldapBinded) {
						$passedValidation = $ldapBinded;
					}
