...
<div id="page-wrapper"> <!-- leave in place for full-screen backgrounds etc -->
	<div class="container"> <!-- -fluid -->
		<h1 class="text-center">UserSpice Dashboard <small>(<?=$user_spice_ver?>)</small></h1>
		<div class="row"> <!-- row for Users, Permissions, Pages, Email settings panels -->
			<h2 style="padding-left: 15px;">Admin Panels</h2>
			<!-- Users Panel -->
			...
			<div class="col-xs-6 col-md-3">
				<div class="panel panel-default">
					...
					<div class="panel-footer">
						<span class="pull-left"><a href='<?=$us_url_root?>users/email_settings.php'>Manage</a></span>
						<span class="pull-right"><i class='fa fa-arrow-circle-right'></i></span>
						<div class="clearfix"></div>
					</div> <!-- /panel-footer -->
				</div> <!-- /panel -->
		...
		<div class="row "> <!-- rows for Info Panels -->
			<h2 style="padding-left: 15px;">Info Panels</h2>
								...
								if($settings->track_guest == 1){ ?>
									...
									<tbody>
										...
										foreach($recentUsers as $user){
											...
											$username=name_from_id($user->user_id);
											$timestamp=date("Y-m-d H:i:s",$user->timestamp);
											$ip=$user->ip;
									...
									echo 'Guest tracking off. Turn "Track Guests" on below for advanced tracking statistics.';
								} ?>
							</table>
						</div>
					</div>
